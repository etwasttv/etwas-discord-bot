...
const command: BotCommand = {
  builder: new SlashCommandBuilder()
    ...
    .setDescription('ファッションブランドを中心に映画、アート、音楽、コスメ、グルメなどを紹介する日本最大級のライフスタイルメディア「ファッションプレス」。おでかけスポットなどの情報も掲載しています。'),
  handler: async interaction => {
    ...
    const item = fashionPressRss.rss.channel.item[Math.floor(Math.random()*fashionPressRss.rss.channel.item.length)];
