...
const command: BotCommand = {
  builder: new SlashCommandBuilder()
    ...
    .addSubcommand((cmd) =>
      cmd.setName('list').setDescription('設定しているタイマー一覧を表示する'),
    ),
  handler: async (interaction) => {
    ...
    if (command === 'set') {
      ...
      const id = await service.setTimer(
        interaction.guild,
        interaction.channel,
        interaction.user,
        timerSeconds,
        mention ? mention.toString() : undefined,
        message,
      );
      ...
      const result = await service.cancelTimer(
        interaction.guild,
        interaction.channel,
        interaction.user,
        timerId,
      ...
      else await interaction.reply('タイマーを解除できませんでした');
    } else if (command === 'list') {
      const timers = await service.getTimerList(
        interaction.guild,
        interaction.user,
      );
      ...
      for (let page = 0; page < pages; page++) {
        const lists = timerLists.slice(
          page * perPage,
          Math.min(timerLists.length, (page + 1) * perPage),
        );
        const msg = `${page + 1}/${pages}` + lists.join('');
        if (!parent) await interaction.reply(msg);
        else await parent.reply(msg);
