...
const command: BotCommand = {
  builder: new SlashCommandBuilder()
    .setName('gigazine')
    .setDescription('日々のあらゆるシーンで役立つ情報を提供するIT系ニュースサイト。毎日更新中。'),
  handler: async interaction => {
    ...
    const entry = gigazineAtom.feed.entry[Math.floor(Math.random()*gigazineAtom.feed.entry.length)];

    const thumbnailMatch = entry.content['#text'].match(/<img src="([^"]+?)".*?>/);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(decode(description.replace(/<a href="([^"]+?)".*?>(.*?)<\/a>/g, '[$2]($1)').replaceAll('<li>', '\n- <li>').replace(/<("[^"]*"|'[^']*'|[^'">])*>/g, '')))
