  ...
  handler: async (interaction) => {
    ...
    if (records.length === 0) {
      await interaction.editReply(
        `${getCategoryString(category)}が観測された地点はありません．`,
      );
      return;
    }

    const maxPointLength = records.reduce(
      (prev, record) => Math.max(prev, record.point.length),
      0,
    );
    ...
    ranking += records
      .map(
        (r) =>
          `${r.rank.toString().padStart(2, ' ')}位 ${r.point.padStart(maxPointLength, '　')} ${r.value} ${unit} (${r.time})`,
      )
      .join('\n');
    ranking += '\n```';
...
async function getAmedasRankingFromYahoo(
  category: Categories,
